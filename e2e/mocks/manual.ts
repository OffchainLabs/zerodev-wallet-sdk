/**
 * Manual-testing runner. Starts the Mockttp proxy with a chosen preset, starts
 * the QA lab, and launches a headed Chromium routed through the proxy — so you
 * drive the real app/SDK by hand against mocked responses.
 *
 * The proxy route, as opposed to the in-app `fetch` rewrite behind
 * `NEXT_PUBLIC_ENABLE_MOCKS`. Both consume the same presets; this one needs no
 * app changes and also catches traffic the app can't reach (the SDK's hardcoded
 * hosts), at the cost of a second process.
 *
 ...
 *   pnpm mock:dev --preset rpcBalances   # apply a named preset
 */
...
const APP_URL = process.env.LAB_APP_URL || 'http://localhost:3000'
...
async function main() {
  ...
  const app: ChildProcess = spawn(
    'pnpm',
    ['--filter', '@zerodev/qa-lab-testing', 'dev'],
