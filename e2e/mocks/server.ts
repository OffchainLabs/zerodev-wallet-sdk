...
// --- Shared singleton, started once per Playwright worker by the auto fixture
// in `test.ts`. Deliberately not a globalSetup: that runs in its own process,
// so `shared` would still be undefined in the process running the specs. ---
...
export function getMockServer(): Mockttp {
  if (!shared) {
    throw new Error(
      'Mock server not started. Mocked specs must import `test` from `mocks/test.ts`, whose worker fixture starts it.',
    )
  }
  return shared
}
