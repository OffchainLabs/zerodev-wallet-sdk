 ...
 */
// The first type argument declares test-scoped fixtures — there are none, so it
// must be an empty object type. `Record<string, never>` looks equivalent but its
// index signature forces every fixture's value to `never`, which makes the
// worker fixture below fail to typecheck.
export const test = base.extend<object, { mockProxy: void }>({
  mockProxy: [
