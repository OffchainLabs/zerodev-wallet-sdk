...
describe('mock server', () => {
  ...
  it('separates same-method calls by bodyIncludes when payload cannot', async () => {
    // The real case this exists for: one `useReadContracts` batch fires
    // `balanceOf`, `decimals` and `symbol` as three `eth_call`s. Every one is
    // `{ method: 'eth_call' }`, so `payload` alone matches all three and the
    // first rule answers for all of them. Only the encoded selector inside
    // `params[0].data` tells them apart.
    ...
    // Interleaved and repeated, for the same reason the test above is: a rule
    // that matches on url+method only would consume and then misroute repeats.
    const results: unknown[] = []
    for (const selector of [BALANCE_OF, DECIMALS, BALANCE_OF, DECIMALS]) {
      const res = await proxyRequest(server.port, RPC_URL, {
        method: 'POST',
        body: {
          jsonrpc: '2.0',
          id: 1,
          method: 'eth_call',
          params: [{ to: '0xcontract', data: `${selector}000000` }, 'latest'],
        },
      })
      results.push(JSON.parse(res.body))
    }
