 ...
 * the proxy, `withMocks` applies the mocks, the proxied browser loads the app.
 *
 * Deliberately login-free, so it stays fast and tells you whether a failure in
 * `user-wallet.mocked.spec.ts` is the harness or the app.
 */
import { expect } from '@playwright/test'
import { userWallet } from '../../mocks/definitions/userWallet.js'
...
test('harness applies mocks and the proxied app loads', async ({ page }) => {
  await withMocks({ mocks: userWallet }, async ({ mockServer }) => {
