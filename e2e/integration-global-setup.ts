/**
 * Fails the integration run immediately when it is misconfigured, rather than
 * letting every test skip itself and reporting green.
 *
 * `ZD_PROJECT_ID` is checked here — not in `scripts/check-integration-ran.mjs`
 * — for two reasons:
 *
...
export default function setup(): void {
  if (!process.env.ZD_PROJECT_ID) {
    throw new Error(
      'ZD_PROJECT_ID is not set, so every integration test would skip itself ' +
